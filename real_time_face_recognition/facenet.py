...
def get_dataset(path, has_class_directories=True):
    ...
    classes = [path for path in os.listdir(path_exp) \
               if os.path.isdir(os.path.join(path_exp, path))]  # 把文件夹的名字添加为类别
    classes.sort()
...
def get_image_paths(facedir):
    ...
    if os.path.isdir(facedir):
        images = os.listdir(facedir)  # 得到每一张图片名
        image_paths = [os.path.join(facedir, img) for img in images]  # 得到了所有图片的绝对路径列表
    return image_paths
