...
class Detector(object):
    ...
    def detect(self, img):
        ...
        # print(inputs.shape) # (448, 448, 3)

        # openCV中默认图片是BGR, 转换成RGB再处理
        inputs = cv2.cvtColor(inputs, cv2.COLOR_BGR2RGB).astype(np.float32)
        # print(inputs.shape)  # (448, 448, 3)
        inputs = (inputs / 255.0) * 2.0 - 1.0  # 将值转换到-1到1范围内
    ...
    def interpret_output(self, output):
        ...
        filter_mat_boxes = np.nonzero(filter_mat_probs)  # 返回非零元素的索引值
        # filter_mat_boxes[0]代表第一维度的索引, 第二第三维度类似
    ...
    def image_detector(self, imname, wait=0):
        ...
        cv2.waitKey(wait)
        cv2.imwrite("./test_image/mayday_out.jpg", image)
